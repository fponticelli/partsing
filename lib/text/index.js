...
exports.upperCaseLetter = exports.regexp(upperCaseLetterPattern)
    .withFailure(error_1.DecodeError.expectedOnce(error_2.Entity.UPPERCASE_LETTER));
exports.upperCaseLetters = function (min, max) {
    if (min === void 0) { min = 1; }
    var message = error_1.DecodeError.expectedAtLeast(min, error_2.Entity.UPPERCASE_LETTER);
    var maxs = max === undefined ? '' : String(max);
...
exports.char = make(function (input) {
    if (input.index < input.input.length) {
        ...
        // no more characters
        return new result_1.DecodeFailure(input, error_1.DecodeError.expectedOnce(error_2.Entity.CHARACTER));
    }
});
    ...
    return make(function (input) {
        if (input.index >= input.input.length) {
            ...
            if (f(char_1)) {
                ...
                return new result_1.DecodeFailure(input, error_1.DecodeError.expectedOnce(error_2.Entity.PREDICATE));
            }
        }
    });
};
...
exports.takeCharWhile = function (f, atLeast) {
    ...
    return make(function (input) {
        ...
        if (index - input.index < atLeast) {
            return new result_1.DecodeFailure(input, error_1.DecodeError.expectedAtLeast(atLeast, error_2.Entity.PREDICATE));
        }
        else {
...
exports.takeCharBetween = function (f, min, max) {
    return make(function (input) {
        ...
        if (counter < min) {
            return new result_1.DecodeFailure(input, error_1.DecodeError.expectedAtLeast(min, error_2.Entity.PREDICATE));
        }
        else {
            return new result_1.DecodeSuccess(__assign({}, input, { index: index }), input.input.substring(input.index, index));
        }
