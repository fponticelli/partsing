...
exports.expect = function (expected, parser) {
    return make(function (source) { return parser.run(source).mapError(function (_) { return expected; }); });
};
...
exports.letter = function () {
    return exports.expect('one letter', exports.regexp(/^[a-z]/i));
};
exports.letters = function (min, max) {
    ...
    return exports.expect(message, exports.regexp(new RegExp("^[a-z]{" + min + "," + maxs + "}", 'i')));
};
exports.digit = function () {
    return exports.expect('one digit', exports.regexp(/^\d/));
};
exports.digits = function (min, max) {
    if (min === void 0) { min = 1; }
    var message = max === undefined ? "at least " + min + " digit(s)" : "between " + min + " and " + max + " digit(s)";
    var maxs = max === undefined ? '' : String(max);
    return exports.expect(message, exports.regexp(new RegExp("^[0-9]{" + min + "," + maxs + "}", '')));
};
exports.whitespace = function () {
    return exports.expect('whitespace', exports.regexp(/^\s+/));
};
exports.optionalWhitespace = function () {
    return exports.expect('optional whitespace', exports.regexp(/^\s*/));
};
...
exports.matchOneOf = function (anyOf) {
    return exports.expect("expected one of `" + anyOf + "`", exports.testChar(function (c) { return anyOf.indexOf(c) >= 0; }));
};
exports.matchNoneOf = function (noneOf) {
    return exports.expect("expected none of `" + noneOf + "`", exports.testChar(function (c) { return noneOf.indexOf(c) < 0; }));
};
