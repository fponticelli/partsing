...
exports.letter = function () {
    return exports.regexp(/^[a-z]/i).withFailure('one letter');
};
exports.letters = function (min, max) {
    ...
    return exports.regexp(new RegExp("^[a-z]{" + min + "," + maxs + "}", 'i')).withFailure(message);
};
exports.digit = function () {
    return exports.regexp(/^\d/).withFailure('one digit');
};
exports.digits = function (min, max) {
    if (min === void 0) { min = 1; }
    var message = max === undefined ? "at least " + min + " digit(s)" : "between " + min + " and " + max + " digit(s)";
    var maxs = max === undefined ? '' : String(max);
    return exports.regexp(new RegExp("^[0-9]{" + min + "," + maxs + "}", '')).withFailure(message);
};
exports.whitespace = function () {
    return exports.regexp(/^\s+/).withFailure('whitespace');
};
exports.optionalWhitespace = function () {
    return exports.regexp(/^\s*/).withFailure('optional whitespace');
};
...
exports.matchOneOf = function (anyOf) {
    return exports.testChar(function (c) { return anyOf.indexOf(c) >= 0; }).withFailure("expected one of `" + anyOf + "`");
};
exports.matchNoneOf = function (noneOf) {
    return exports.testChar(function (c) { return noneOf.indexOf(c) < 0; }).withFailure("expected none of `" + noneOf + "`");
};
